...
function recexplo.create_global_table(player_index)
	...
	if not global[player_index].selctet_product_signal then global[player_index].selctet_product_signal = { type = "item", name = "piercing-rounds-magazine"} end --crude-oil
	if not global[player_index].display_mod then global[player_index].display_mod = "recipe" end --recipe, where_used
	...
	if not global[player_index].history.length then global[player_index].history.length = 1 end	
	if not global[player_index].history.pos then global[player_index].history.pos = 1 end
	if not global[player_index].history[1] then global[player_index].history[1] = {} end
	if not global[player_index].history[1].signal then global[player_index].history[1].signal = {} end	
	if not global[player_index].history[1].signal.name then global[player_index].history[1].signal.name = global[player_index].selctet_product_signal.name end	
	if not global[player_index].history[1].signal.type then global[player_index].history[1].signal.type = global[player_index].selctet_product_signal.type end	
	if not global[player_index].history[1].display_mod then global[player_index].history[1].display_mod = global[player_index].display_mod end	
