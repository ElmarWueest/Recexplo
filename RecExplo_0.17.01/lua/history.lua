...
function recexplo.history.delete_pos(history, delete_pos)
	...
	if history.length == delete_pos then
		...
		history.list[history.length] = nil
		history.length = history.length -1
    end
...
--[[function recexplo.history.load(player_index, pos)
    ...
    if pos > 0 then
	    global[player_index].selctet_product_signal.name = history.list[pos].signal.name
	    global[player_index].selctet_product_signal.type = history.list[pos].signal.type
        global[player_index].display_mode = history.list[pos].display_mode
	else
